module AjaxfulRating # :nodoc:
  ...
  module InstanceMethods
    ...
    # Instance's total stars rates.
    def stars_rates(stars, dimension = nil)
      conditions = {:stars => stars}
      if dimension
        send("#{dimension}_rates").all(:conditions => ['stars = ?', stars]) 
      else
        rates_without_dimension.all(:conditions => ['stars = ?', stars]) 
      end
    end
  ...
  module SingletonMethods
    ...
    # Finds rates.
    def find_rates(dimension = nil, attr_name = nil, attr_value = nil)
      sql = "SELECT * FROM rates WHERE "

      conditions = {:rateable_type => self.base_class.name }
      conditions.merge!({:dimension => dimension.to_s}) if dimension
      conditions.merge!({attr_name => attr_value}) if attr_name and attr_value 

      sql << sanitize_sql_for_conditions(conditions, 'rates')

      find_by_sql(sql)
    end

    #Find all records rated by user for any dimension
    def find_records(user_id)
      find_by_sql("SELECT DISTINCT base.* FROM rates INNER JOIN #{self.base_class.table_name} base ON rates.rateable_id = base.id WHERE rates.rater_id = #{user_id} AND rates.rateable_type = '#{self.base_class}'")
    end
